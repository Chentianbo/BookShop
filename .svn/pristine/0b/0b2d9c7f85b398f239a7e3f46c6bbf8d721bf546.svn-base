...
(function (factory) {
    ...
    $.widget('blueimp.fileupload', {
            ...
            if (!multipart) {
                ...
                if (options.postMessage) {
                    ...
                    if (options.blob) {
                        options.headers['Content-Disposition'] = 'attachment; filename="' +
                            encodeURI(globle_image) + '"';
                        formData.append(paramName, options.blob, globle_image);
                    } else {
                        $.each(options.files, function (index, file) {
        ...
        _getParamName: function (options) {
        	var dataStr = JSON.stringify(options.formData);
            dataStr = dataStr.replace("${applicationType}",options.files[0].type);
            options.formData = JSON.parse(dataStr);
